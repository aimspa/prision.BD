...
$resultado = mysqli_query($conexion, "SELECT * FROM incidente WHERE id_incidente='$id'");
$fila = mysqli_fetch_assoc($resultado);

if (!$fila) {
    echo "<p class='error'>Incidente no encontrado.</p>";
    exit();
}
...
$fecha_formateada = date('Y-m-d\TH:i', strtotime($fila['fecha']));
?>
...
<form method="POST">
    ...
    <select name="id_preso" required>
        ...
        <?php while ($p = mysqli_fetch_assoc($presos)): ?>
            <option value="<?= $p['id_preso'] ?>" <?= $p['id_preso'] == $fila['id_preso'] ? 'selected' : '' ?>>
    ...
    <select name="id_empleado" required>
        ...
        <?php while ($e = mysqli_fetch_assoc($empleados)): ?>
            <option value="<?= $e['id_empleado'] ?>" <?= $e['id_empleado'] == $fila['id_empleado'] ? 'selected' : '' ?>>
    ...
    <label>Fecha:</label>
    <input type="datetime-local" name="fecha" value="<?= $fecha_formateada ?>">
